...
import { cashFlowService, currencyService, AccountService, PaymentService } from '../../services'
...
const AddCashFlow = ({ dispatch, type, match, history }) => {
    ...
    useEffect( () => {
        currencyService.getAll().then( response => setCurrency(response.data) )
        PaymentService.getAll().then( response => setMethod(response.data) )
        AccountService.getAll().then( response => setAccount(response.data) )
    ...
    return (
        <div className="form-wrapper mx-auto py-4 px-3">
            { type === 0 ? <h4 className="heading mb-3">Cash flows entry</h4> : null }
            ...
            <form name="cashflow" className="pre-disabled" onSubmit={submitHandler}>
                ...
                { fields.map (
                    ...
                    : <Field key={field.name} field={field} value={inputs[field.name]} handleChange={_handleChange} /> 
                ) }
